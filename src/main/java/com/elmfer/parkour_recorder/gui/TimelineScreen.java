...
public class TimelineScreen extends GuiScreen
{
	...
	protected FontRenderer font = minecraft.fontRenderer;
	...
	public void drawScreen(MatrixStack stack, int mouseX, int mouseY, float partialTicks)
	{
		...
		if(settingsBody.getHeight() > smallMargin * 2)
		{
			settingsBody.pushMatrix(false);
			{
				...
				{
					GuiSlider slider = (GuiSlider) buttons.get(8);
		...
		title.pushMatrix(false);
		{
			...
			GraphicsHelper.drawCenteredString(minecraft.fontRenderer, I18n.format("gui.timeline"), title.getWidth() / 2, title.getHeight() / 2 - mc.fontRenderer.FONT_HEIGHT / 2, 0xFFFFFFFF);
		}
		title.popMatrix();
	}
	...
	private void renderTaskbar(GuiViewport title, GuiViewport taskBar, int mouseX, int mouseY, float partialTicks)
	{
		...
		{	
			...
			GraphicsHelper.drawString(font, recordingName, title.right + margin * 2, taskBar.getHeight() / 2 - minecraft.fontRenderer.FONT_HEIGHT / 2, nameColor);
			
			GraphicsHelper.vLine(title.right + margin * 3 + font.getStringWidth(recordingName), smallMargin, taskBar.getHeight() - smallMargin * 2, getIntColor(0.4f, 0.4f, 0.4f, 0.5f));
			...
			start.setX(title.right + margin * 4 + font.getStringWidth(recordingName));
	...
	{	
		...
		{	
			...
			for(GuiButton control : buttonControls)
			{
				...
				control.setY(smallMargin);
				control.setX(i * (size + smallMargin) + smallMargin);
				i++;
			}
			...
			settings.setX(atEnd.x() + size + margin);
			pause.setX(atBegginning.x() + size + smallMargin);;
