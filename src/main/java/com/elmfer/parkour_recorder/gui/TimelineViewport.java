...
public class TimelineViewport extends AbstractGui implements IGuiEventListener
{
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks, GuiViewport viewport)
	{
		...
		viewport.pushMatrix(true);
		{
			...
			GraphicsHelper.fillGradient(0, 0, viewport.getWidth(), viewport.getHeight(), fade1, fade2);
			GraphicsHelper.fillGradient(0, 0, numberLine.getWidth(), numberLine.getHeight(), grad1, grad2);
			...
			if(delta == 0) delta = 5;
			{
				for(float i = 0.0f; i < duration; i += delta)
				{
					...
					GraphicsHelper.drawCenteredString(font, Integer.toString((int) i), (int) numberX, stringCenterY, 0xFFFFFFFF);
			...
			pointer.pushMatrix(false);
			{
				...
				GraphicsHelper.fillGradient(0, 0, pointer.getWidth(), pointer.getHeight(), blue1, blue2);
				GraphicsHelper.drawCenteredString(font, Integer.toString((int)framePos), pointer.getWidth() / 2, stringCenterY, 0xFFFFFFFF);
				GraphicsHelper.vLine(pointer.getWidth() / 2, pointer.getHeight(), viewport.getHeight(), blue1);
			}
			pointer.popMatrix();
