...
public class TimelineViewport extends AbstractGui implements IGuiEventListener
{
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks, GuiViewport viewport)
	{
		...
		viewport.pushMatrix(true);
		{
			...
			fillGradient(0, 0, viewport.getWidth(), viewport.getHeight(), FADE_1, FADE_2);
			fillGradient(0, 0, numberLine.getWidth(), numberLine.getHeight(), GRAD_1, GRAD_2);
			...
			vLine((int) pointerPos, pointer.getHeight() - 1, viewport.getHeight(), BLUE_1);
			pointer.pushMatrix(false);
			{
				...
				fillGradient(0, 0, pointer.getWidth(), pointer.getHeight(), BLUE_1, BLUE_2);
