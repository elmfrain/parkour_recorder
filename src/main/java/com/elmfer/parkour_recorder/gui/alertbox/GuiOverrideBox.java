...
public class GuiOverrideBox extends GuiAlertBox 
{
	...
	protected void doDrawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		viewport.pushMatrix(false);
		{
			...
			saveAsNew.setX(BUTTON_WIDTH + MARGIN);
			cancel.setX(viewport.getWidth() - BUTTON_WIDTH);
			override.setY(GuiStyle.Gui.buttonHeight() + MARGIN);
			saveAsNew.setY(GuiStyle.Gui.buttonHeight() + MARGIN);
			cancel.setY(GuiStyle.Gui.buttonHeight() + MARGIN);
			
			//Show message
			String[] message = this.message.split("\n");
			int i = 0;
			for(String line : message) GraphicsHelper.drawCenteredString(mc.fontRenderer, line, viewport.getWidth() / 2, i++ * mc.fontRenderer.FONT_HEIGHT, 0xFFFFFFFF);
