...
abstract public class Window extends Widget {
	...
	public void update(SidedUpdate side)
	{
		if(side == SidedUpdate.CLIENT)
		{
			if(shouldClose)
			{
				...
				if(LOADED_WINDOWS.size() > 0) Widget.setCurrentZLevel(LOADED_WINDOWS.get(LOADED_WINDOWS.size() - 1).getzLevel());
				else Widget.setCurrentZLevel(0);
			}
		}
	}
	...
	public void draw()
	{
		...
		{
			UIrender.drawGradientRect(Direction.TO_BOTTOM, -smallMargin, box.getHeight(), boxWidth + smallMargin, box.getHeight() + 10.0f, -1946157056, 0);
			UIrender.drawGradientRect(-smallMargin, -smallMargin, box.getWidth() + smallMargin, box.getHeight() + smallMargin, -12632257, -14277082);
		}
		box.popMatrix();
		
		title.pushMatrix(false);
		{
			UIrender.drawGradientRect(Direction.TO_RIGHT, 0, 0, title.getWidth(), title.getHeight(), -1728053248, 1275068416);
	...
	/** Creates the window within this method to correctly set the z values of all widgets (like buttons)
	...
	public static void drawWindows()
	{
		...
		for(Window window : LOADED_WINDOWS)
		{
			...
			{
				GlStateManager.translate(4 * i, 4 * i++, 0);
				window.draw();
			}
	...
	public static void closeWindows()
	{
		...
		LOADED_WINDOWS.forEach((w) -> {w.setShouldClose(true);});
	}
