...
public class Recording implements List<Frame> {
    ...
    /**
     * Filters out files that doesn't end with the {@code FILE_EXTENSION} or if the
     * format is unknown.
     **/
    public static final FileFilter FILE_FILTER = new FileFilter() {
        ...
        public boolean accept(File pathname) {
            if (pathname.getAbsolutePath().endsWith(FILE_EXTENSION)) {
                ...
                SaveFormat format = SaveFormat.getFormatFromID(header.FORMAT_ID);
    ...
    /**
     * Ensures that when the recordings are saved, its data has the correct byte
     * order.
     **/
    ...
    public Recording(Vec3d startingPos) {
        initPos = startingPos;
        this.name = null;
    }
    ...
    public String toString() {
        ...
        Vec3d initPos = new Vec3d(initPosX, initPosY, initPosZ);
        ...
        Vec3d lastPos = new Vec3d(lastPosX, lastPosY, lastPosZ);
        ...
        s += String.format("%s%s%.2f %.2f %.2f\n\n", I18n.translate("com.prmod.starting_position"), tab, initPos.x,
                initPos.y, initPos.z);
        s += String.format("%s%s%.2f %.2f %.2f\n\n", I18n.translate("com.prmod.ending_position"), tab, lastPos.x,
                lastPos.y, lastPos.z);
        return s;
    }
    ...
    public byte[] serialize() {
        ...
        header.put((byte) 0);
        header.putInt(SaveFormat.LATEST.ID);

        // Fill recording data
        recording.putInt(frames.size());
        recording.putInt(checkpoints.size());
        recording.putDouble(initPos.x);
        recording.putDouble(initPos.y);
        recording.putDouble(initPos.z);
        recording.putDouble(lastPos.x);
        recording.putDouble(lastPos.y);
        recording.putDouble(lastPos.z);
        for (Frame frame : frames)
            recording.put(frame.serialize());
        for (Checkpoint checkpoint : checkpoints)
            recording.put(checkpoint.serialize());
     ...
     * @param mustSave Determines if it's mandatory to save. If set to
     *                 {@code false}, recording will not save if it is not
     *                 originally loaded from a file.
     * @param override Override the original recording's file. If set to
     *                 {@code true}, recording will save to the orignal file, if
     *                 any. Else, it saves to a new file.
     **/
    public void save(boolean threaded, boolean mustSave, boolean override) {
        if (!mustSave && recordFile == null)
            return;

        // Threaded saving; used for saving while adding or removing checkpoints
        if (threaded)
            new SavingThread(this, override).start();
        else {
            save(this, override);
        }
    }
    ...
    /**
     * Delete recording file. Returns {@code false} if recording wasn't loaded from
     * a file, and it was unable to delete.
     **/
    ...
    public static String getCurrentWorldName(MinecraftClient mc) {
        if (mc.getServer() != null) {
            return mc.getServer().getSaveProperties().getLevelName();
        } else
            return mc.getCurrentServerEntry().name
                    + (Config.isHiddenIp() ? "" : ": " + mc.getCurrentServerEntry().address);
    }

    private static String getWorldPath(MinecraftClient mc) {
        if (mc.getServer() != null) {
            return "local/" + mc.getServer().getSaveProperties().getLevelName();
        } else
            return "servers/" + mc.getCurrentServerEntry().address.replace('.', '-').replace(':', '_');
    }
    ...
    @Override
    public void add(int index, Frame element) {
        frames.add(index, element);
    }

    @Override
    public boolean addAll(int index, Collection<? extends Frame> c) {
    ...
    public Recording subList(int fromIndex, int toIndex) {
        ...
        List<Frame> subList = frames.subList(fromIndex, toIndex);
        ...
        subedRecording.lastPos = new Vec3d(subList.get(lastIndex).posX, subList.get(lastIndex).posY,
        ...
        subedRecording.checkpoints.removeIf((Checkpoint c) -> {
            return c.frameNumber < fromIndex || toIndex < c.frameNumber;
        });
    ...
    private static void save(Recording recording, boolean override) {
        ...
        boolean shouldUseOriginalName = recording.originalFile != null && override && !recording.renamed;
        recording.name = recording.name == null ? recordFileName
                : shouldUseOriginalName ? recording.originalName : recording.name;
        recording.renamed = false;

        // Determine file to save to
        File worldDir = new File(SAVING_DIRECTORY.getPath() + '/' + worldName);
        File file = override && recording.originalFile != null ? recording.originalFile
                : new File(worldDir.getPath() + '/' + recordFileName + FILE_EXTENSION);
    ...
    private static class SavingThread extends Thread {
        ...
        @Override
        public void run() {
            save(recording, override);
        }
    }

    /** Internal deserializers. **/
    private static abstract class Deserializer {
        public abstract Recording deSerialize(ByteBuffer buffer)
        ...
        /**
         * Stores the name and format-id of a saved recording. All saved files, dispite
         * the format (the mod version used to save), starts with the name and
         * format-id, the header.
         **/
        ...
        public static Deserializer V1_0_0_0 = new Deserializer() {
            ...
            public Recording deSerialize(ByteBuffer buffer) {
                ...
                for (int i = 0; i < header.FORMAT_ID; i++)
                    newRecording.add(Frame.deSerialize(buffer, format));
        ...
        public static Deserializer V1_0_1_0 = new Deserializer() {
            ...
            public Recording deSerialize(ByteBuffer buffer) {
                ...
                try {
                    decompressor.inflate(recordingData);
                ...
                Vec3d initPos = new Vec3d(recording.getDouble(), recording.getDouble(), recording.getDouble());
                Vec3d lastPos = new Vec3d(recording.getDouble(), recording.getDouble(), recording.getDouble());
                ...
                for (int i = 0; i < NUM_FRAMES; i++)
                    newRecording.add(Frame.deSerialize(recording, format));

                // Get checkpoints
                for (int i = 0; i < NUM_CHECKPOINTS; i++)
                    newRecording.checkpoints.add(Checkpoint.deSerialize(recording, format));
