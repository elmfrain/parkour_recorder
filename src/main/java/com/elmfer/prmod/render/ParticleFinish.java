    ...
    public void buildGeometry(VertexConsumer p_107261_, Camera renderInfo, float partialTicks) {
        ...
        RenderSystem.getModelViewStack().push();
        {
            ...
            AxisAngle4f yAngle = new AxisAngle4f(
                    (float) Math.toRadians(angle) + (float) (Math.toRadians(-Math.pow(expiredAmount, 2.0) * 720.0)), 0,
                    1, 0);
            AxisAngle4f zAngle = new AxisAngle4f((float) Math.toRadians(Math.sin(ticks * Math.PI / 20.0) * 10), 0, 0,
                    1);
