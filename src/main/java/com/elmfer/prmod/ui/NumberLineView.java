...
import com.elmfer.prmod.ui.TimelineView.SessionType;
...
public class NumberLineView extends Widget {
    ...
    public void draw() {
        ...
        int stringCenterY = (int) numberLineViewport.getHeight() / 2 - font.fontHeight / 2;

        // Styling constants
        final int FADE_HEIGHT = GuiStyle.Gui.fadeHeight();
        final int FADE_1 = GraphicsHelper.getIntColor(GuiStyle.Gui.fade1());
        ...
        mainViewport.pushMatrix(true);
        {
            ...
            UIRender.drawGradientRect(0, 0, numberLineViewport.getWidth(), numberLineViewport.getHeight(), GRAD_1,
                    GRAD_2);

            // Determine spacing of time markers
            int nbTimeMarkers = Math.min((int) mainViewport.getWidth()
                    / (TimelineView.timeStampFormat.AVERAGE_LENGTH * NUM_CHAR_WIDTH + MARGIN * 2), 10);
                ...
                delta = 5;
            {
                for (float i = 0.0f; i < duration; i += delta) {
                    ...
                    UIRender.drawString(Anchor.TOP_CENTER, TimelineView.timeStampFormat.getTimeStamp((int) i),
                            (int) numberX, stringCenterY, 0xFFFFFFFF);
                    UIRender.drawVerticalLine((int) numberX, numberLineViewport.bottom, mainViewport.bottom,
                            2137417318);
                }
            }
            ...
            if (EventHandler.session instanceof PlaybackSession)
                for (Checkpoint c : ((PlaybackSession) EventHandler.session).recording.checkpoints)
                    renderCheckpointMarker(c, (int) numberLineViewport.getHeight(), (int) mainViewport.getWidth(),
                            (int) mainViewport.getHeight());

            // Position pointer
            String timeStamp = TimelineView.timeStampFormat.getTimeStamp(framePos);
            float pointerWidth = (font.getWidth(timeStamp)) / 2.0f + SMALL_MARGIN;
            ...
            float pointerHeadPos = Math.max(pointerWidth,
                    Math.min(pointerPos, numberLineViewport.getWidth() - pointerWidth));
            pointer.left = (int) (pointerHeadPos - pointerWidth);
            pointer.right = (int) (pointerHeadPos + pointerWidth);

            // Render pointer
            UIRender.drawGradientRect(Direction.TO_RIGHT, (int) pointerPos - FADE_HEIGHT / 4, pointer.getHeight(),
                    (int) pointerPos, mainViewport.getHeight(), FADE_2, FADE_1);
            UIRender.drawGradientRect(Direction.TO_RIGHT, (int) pointerPos + 1, pointer.getHeight(),
                    (int) pointerPos + FADE_HEIGHT / 4 + 1, mainViewport.getHeight(), FADE_1, FADE_2);
            ...
            pointer.pushMatrix(false);
            {
                ...
                UIRender.drawGradientRect(Direction.TO_RIGHT, pointer.getWidth(), 0,
                        pointer.getWidth() + FADE_HEIGHT / 2, pointer.getHeight(), FADE_1, FADE_2);
            ...
            if (!parentWidget.session.isActive())
                UIRender.drawRect(0, 0, mainViewport.getWidth(), mainViewport.getHeight(),
    ...
    private void positionPointer(int cursorX, int cursorY) {
        if (numberLineViewport == null || mainViewport == null)
            return;
        // Begin dragging pointer if mouse is pressed on it
        if (mainViewport.isHovered(cursorX, cursorY) && leftButtonIsPressed
                && parentWidget.session == SessionType.REPLAY
                && (parentWidget.timeline.isPaused() || parentWidget.timeline.hasStopped()) && onCurrentZlevel())
            pointerIsDragging = true;

        // Set timeline position equals to cursor if dragging
        if (pointerIsDragging)
            parentWidget.timeline.setFracTime(
                    (cursorX - numberLineViewport.getAbsoluteLeft() * 1.0f) / numberLineViewport.getWidth());
    }
    ...
    public void onMouseScroll(int scrollAmount) {
        ...
        if (mainViewport != null && mainViewport.isHovered(mouseX, mouseY)
                && (parentWidget.timeline.isPaused() || parentWidget.timeline.hasStopped()) && onCurrentZlevel()) {
            ...
            if (0 < scrollAmount) // Goto next frame
            {
                ...
                parentWidget.timeline
                        .setFracTime(currentFrame / (parentWidget.timeline.getDuration() * 20.0) + oneTick / 10.0);
