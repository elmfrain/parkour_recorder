    ...
    public void onCharTyped(int charTyped) {
        if (focused) {
            if (secondCursorPosition == cursorPosition) {
                setText(getText().substring(0, cursorPosition) + (char) charTyped
                        + getText().substring(cursorPosition));
                cursorPosition++;
            } else {
    ...
    public void draw() {
        ...
        Stencil.pushStencilState();
        {
            ...
            if (!getIcon().isEmpty()) {
                ...
                UIRender.drawIcon(getIcon(), iconX, y + height / 2, DEFAULT_ICON_SCALE,
                        GraphicsHelper.getIntColor(0.7f, 0.7f, 0.7f, iconTransparency));
            } else {
                float titleOffset = (float) titlePosition.getValue();
                float titleTransparency = (float) titleOpacity.getValue();

                UIRender.drawString(Anchor.MID_LEFT, title, x + SPACING - titleOffset, y + height / 2,
            ...
            RenderSystem.getModelViewStack().push();
            {
                ...
                if (focused) {
                    ...
                    if (secondCursorPosition != cursorPosition) {
                        float secondCursorXCoord = UIRender.getStringWidth(getText().substring(0, secondCursorPosition))
                                + SPACING;
                        float blueBoxXCoord = cursorXCoord;
                        float blueBoxWidth = secondCursorXCoord - cursorXCoord;

                        UIRender.drawRect(blueBoxXCoord + x, y + SPACING, blueBoxXCoord + x + blueBoxWidth,
                                y + height - SPACING, -1776726785);
                    }
                    if (showCursor) {
                        UIRender.drawRect(cursorXCoord + x, y + SPACING, cursorXCoord + x + 1, y + height - SPACING,
                                0xFFFFFFFF);
                    }
                }
